...
import { BehaviorSubject, combineLatest, from, Observable, Subject, Subscription, takeUntil } from 'rxjs';
...
export class PageComponent implements OnInit, OnDestroy {
  ...
  onBackPressed() {
    console.log(`PageComponent.onBackPressed`);
    ...
    if (currentIndex > 0) {
      ...
      console.log("Current token:", this.accessTokenService.getCurrentToken());
  ...
  onMarqueeRightClick(event: MouseEvent, marquee: Marquee) {
    ...
    if (this.mode === 'select') {
      this.router.navigate([
        `/diary/${this.diary.id}/${this.page.id}/${marquee.id}`
      ]);
    }
  }
}
