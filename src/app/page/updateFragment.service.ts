...
import { Fragment, AddFragmentRequest, UpdateFragmentRequest } from '../model/fragment/fragment';
...
export class UpdateFragmentService {
  ...
  updateFragment(diary: Diary, page: Page, fragment: Fragment): Promise<number> {
  ...
  connected(client: mqtt.MqttClient, config: Config, diary: Diary, page: Page, fragment: Fragment, resolve: (value: number) => void, reject: (reason?: any) => void) {
    console.log(`FragmentService.updateFragment.connected`);
    ...
    client.subscribe(replyTopic, { qos: 1 }, (err) => {
      ...
      const payload = {
        ...
        args: new UpdateFragmentRequest(
          fragment.id,
          fragment.pageId,
          fragment.x,
          fragment.y,
